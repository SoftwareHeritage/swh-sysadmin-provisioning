...
locals {
  ...
  project_permission_tuples = toset(flatten([
    for cluster_name in keys(var.project_permissions) : [
      for project_name in keys(var.project_permissions[cluster_name]) : [
        for role_template_id in keys(var.project_permissions[cluster_name][project_name]) : [
          for user_name in var.project_permissions[cluster_name][project_name][role_template_id] : {
...
output "project_permissions" {
  ...
  description = "Set of {cluster_id, cluster_name, project_id, role_name_id, user_id} derived from var.project_permissions"
}