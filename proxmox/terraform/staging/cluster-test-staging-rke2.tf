...
module "rancher-node-test-rke2-worker4" {
  source      = "../modules/node_bpg"
  config      = local.config
  hypervisor  = "uffizi"
  onboot      = false
  hostname    = "rancher-node-test-rke2-worker4"
  description = "kubernetes worker for computations (e.g. loader, lister, ...)"
  tags        = ["test-staging-rke2"]
  vmid        = 109
...
output "rancher-node-test-rke2-worker4_summary" {
  value = module.rancher-node-test-rke2-worker4.summary
}
